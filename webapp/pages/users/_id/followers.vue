<template>
  <v-container>
    <v-card>
      <card-title> {{ user.username }}'s followers </card-title>
      ...
      <v-card-text v-else>
        {{ user.username }} isn't followed by anyone yet.
      </v-card-text>
    </v-card>
  </v-container>
</template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const { fetch, fetchState } = useFetch(async () => {
      const response = await $axios.$post(`/graphql`, {
        query: `
          query ($userId: UUID!, $page: Int!) {
            followers(userId: $userId, page: $page) {
              content {
                user {
                  id
                  username
      ...
      pageResponse.value = response.data.followers
    })
